...
// 🚨 CẢNH BÁO: HÃY CẬP NHẬT LẠI TOKEN NẾU GẶP LỖI KẾT NỐI
const TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..SHOQDpPY5-wEhxXOLY5QHoLAYdVh4Cm0lywflDtCQYw";
...
// 1. Thuật toán Cân bằng Tần suất (Cơ sở)
...
// 2. Thuật toán Markov Cổ điển (Order 3)
...
// 3. Thuật toán N-gram (So khớp chuỗi)
...
// 4. Neo-Pattern Recognition (So khớp mẫu linh hoạt)
...
// 5. AI Phân Tích Siêu Sâu và Kĩ (Super Deep Analysis & Mean Reversion)
...
// 6. Mô hình Biến áp Transformer (Tối ưu hóa So khớp chuỗi dài)
...
// 7. AI Bẻ Cầu và Bắt Cầu Siêu Chuẩn (Super Bridge Predictor)
function algoG_SuperBridgePredictor(history) {
    const runs = extractFeatures(history).runs;
    if (runs.length < 2) return null;
    const lastRun = runs.at(-1);

    // AI Bắt Cầu Siêu Chuẩn (Cầu dài từ 4 trở lên)
    if (lastRun.len >= 4) {
        // Tăng cường độ tin cậy khi cầu đang chạy
        return lastRun.val;
    }

    // AI Bẻ Cầu Siêu Chuẩn (Phát hiện cầu 1-1 bị break hoặc bẻ bệt)
    if (runs.length >= 4) {
...
// 8. FULL THUẬT TOÁN MARKOV THÍCH ỨNG (Adaptive Markov)
...
const ALL_ALGS = [{
    ...
    id: 'e_transformer', // Transformer Model
    fn: algoE_Transformer
}, {
    id: 'g_super_bridge_predictor', // AI Bẻ Cầu & Bắt Cầu
    fn: algoG_SuperBridgePredictor
}, {
    id: 'h_adaptive_markov', // AI Thích Ứng
...
class SEIUManager {
    ...
    // Đã xóa logic thống kê (tong_thang/thua)
    ...
    pushRecord(record) {
        ...
        console.log(`📥 Phiên mới ${parsed.session} → ${parsed.result}. Dự đoán phiên ${parsed.session + 1} là: ${this.currentPrediction.prediction}.`);
    }
...
// --- PHẦN 2: API SERVER VÀ KẾT NỐI WEBSOCKET ---
...
app.get("/api/taixiu/sunwin", async () => {
    ...
    return {
        ...
        // Ghi chú: Đã xóa thong_ke_hieu_suat_he_thong và tong_thang/thua
    };
});
...
// ❌ ĐÃ XÓA ENDPOINT /api/taixiu/stats THEO YÊU CẦU
...
// --- SERVER START & WEBSOCKET (Giữ nguyên) ---
const start = async () => {
    try {
        await app.listen({
            port: PORT,
...
`;
    ...
    console.log(`   ➜ GET /api/taixiu/sunwin   → http://${publicIP}:${PORT}/api/taixiu/sunwin`);
};

start();

// --- WEBSOCKET HANDLERS (Giữ nguyên) ---
